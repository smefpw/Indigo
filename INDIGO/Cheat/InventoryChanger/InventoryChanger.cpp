...
void AddItem(CMsgSOCacheSubscribed::SubscribedType* Object, int index, int itemDefIndex, int rarity, int paintKit, int seed, float wear, std::string name, int quality)
{
	CSOEconItem Skin;
	...
	//Skin.set_rarity(rarity); // let the game decides rarity. Why? Because you don't want to fuck up like Alex did.
...
void CInventoryChanger1::PostRetrieveMessage(uint32_t* punMsgType, void* pubDest, uint32_t cubDest, uint32_t* pcubMsgSize)  // RANK CHANGER
{
	...
	if (MessageType == k_EMsgGCCStrike15_v2_MatchmakingGC2ClientHello)
	{
		...
		Message.set_player_level(Settings::InvChanger::Profile_Info_Level); // PLAYER LEVEL
		Message.set_player_cur_xp(Settings::InvChanger::Profile_Info_XP); // PLAYER XP

																		  // TournamentPlayer

		PlayerRankingInfo* Ranking = Message.mutable_ranking();
		Ranking->set_account_id(Interfaces::SteamUser()->GetSteamID().GetAccountID());
		Ranking->set_rank_id(Settings::InvChanger::Profile_Info_Rank); // PLAYER RANK 1-18 (18 = Global Elite)
		Ranking->set_wins(Settings::InvChanger::Profile_Info_Win);  // PLAYER WINS IN MATCHMAKING

		PlayerCommendationInfo* Commendation = Message.mutable_commendation();
		Commendation->set_cmd_friendly(Settings::InvChanger::Profile_Info_Friendly);  //RECOMMENDATIONS FRIENDLY
		Commendation->set_cmd_leader(Settings::InvChanger::Profile_Info_Leader);    //RECOMMENDATIONS LEADER
		Commendation->set_cmd_teaching(Settings::InvChanger::Profile_Info_Teacher);  //RECOMMENDATIONS TEACHING
...
void CInventoryChanger::ApplyMedals(CMsgSOCacheSubscribed::SubscribedType* pInventoryCacheObject)
{
	...
	for (uint32_t MedalIndex : Settings::InvChanger::medals)
	{
		...
		int64_t ms = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();
